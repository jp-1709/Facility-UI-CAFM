/**
 * AssetReporting.tsx  ·  CAFM Facility-UI
 * ─────────────────────────────────────────────────────────────────────
 * Dynamic report viewer for Frappe Script Reports.
 * Reports implemented:
 *   • Assets Summary       (assets_summary.py)
 *   • Assets Register      (assets_register.py)
 *
...
import {
  ...
  CalendarDays, Settings, ZapOff,
  AlertCircle as AlertIcon, Wrench as WrenchIcon,
} from "lucide-react";
...
function useRptStyles() {
  useEffect(() => {
    if (document.getElementById("rpt-css-asset")) return;
    const s = document.createElement("style"); s.id = "rpt-css-asset"; s.textContent = RPT_CSS;
...
══════════════════════════════════════════════════════════════ */
const ASSETS_SUMMARY_FILTERS: FilterDef[] = [
...
const ASSETS_REGISTER_FILTERS: FilterDef[] = [
  { fieldname: "from_date",           label: "From Date",           fieldtype: "Date"   },
  { fieldname: "to_date",             label: "To Date",             fieldtype: "Date"   },
  { fieldname: "asset_code",          label: "Asset Code",          fieldtype: "Link",   options: "CFAM Asset" },
  { fieldname: "client_code",         label: "Client",              fieldtype: "Link",   options: "Client" },
  { fieldname: "property_code",       label: "Site",                fieldtype: "Link",   options: "Property" },
  { fieldname: "asset_master_category", label: "Category",         fieldtype: "Select",
    options: "\nHVAC\nElectrical\nPlumbing\nFire Safety\nElevator\nGenerator\nWater Treatment\nLandscaping\nCleaning\nPest Control\nSecurity" },
  { fieldname: "asset_status",        label: "Status",              fieldtype: "Select",
    options: "\nActive\nInactive\nUnder Maintenance\nScrapped\nLost" },
];
...
const ASSET_REPORTS: ReportDef[] = [
  {
    ...
    accent: "#dc2626",
    filters: ASSETS_SUMMARY_FILTERS,
    description: "Aggregated asset metrics including total assets, active assets, and breakdown statistics by client and site.",
  },
  {
    id: "register",
    ...
    accent: "#ea580c",
    filters: ASSETS_REGISTER_FILTERS,
    description: "Complete asset register with detailed information, warranty tracking, and status monitoring.",
  },
];
...
const ASSET_STATUS_STYLES: Record<string, string> = {
  ...
  "Lost":               "bg-purple-100 text-purple-700 border-purple-200",
};

/* ══════════════════════════════════════════════════════════════
   HELPER: format cell value for display
════════════════════════════════════════════════════════════════ */
function formatAssetCellValue(col: ReportColumn, val: string | number | null): { text: string; badge?: string } {
  if (val === null || val === undefined || val === "") return { text: "—" };
  const s = String(val);

  if (col.fieldname === "asset_status") return { text: s, badge: ASSET_STATUS_STYLES[s] };
...
/* ══════════════════════════════════════════════════════════════
   SUMMARY STAT CARDS (for Assets Summary report)
════════════════════════════════════════════════════════════════ */
function AssetsSummaryKPIs({ data }: { data: ReportRow[] }) {
  const totals = useMemo(() => {
    let totalAssets = 0, activeAssets = 0, criticalAssets = 0, pmDue = 0, breakdownThisMonth = 0;
    for (const r of data) {
      totalAssets       += Number(r.total_assets        || 0);
      activeAssets     += Number(r.active_assets      || 0);
      criticalAssets   += Number(r.critical_assets    || 0);
      pmDue           += Number(r.pm_due             || 0);
      breakdownThisMonth += Number(r.breakdown_this_month || 0);
    }
    return { totalAssets, activeAssets, criticalAssets, pmDue, breakdownThisMonth };
  }, [data]);

  const cards = [
    { label: "Total Assets",    value: totals.totalAssets,       icon: <Package        className="w-4 h-4" />, color: "#dc2626", bg: "#fef2f2" },
    { label: "Active Assets",   value: totals.activeAssets,      icon: <CheckCircle2   className="w-4 h-4" />, color: "#059669", bg: "#ecfdf5" },
    { label: "Critical Assets", value: totals.criticalAssets,    icon: <AlertIcon      className="w-4 h-4" />, color: "#ea580c", bg: "#fff7ed" },
    { label: "PM Due",          value: totals.pmDue,             icon: <Clock          className="w-4 h-4" />, color: "#7c3aed", bg: "#f5f3ff" },
    { label: "Breakdowns This Month", value: totals.breakdownThisMonth, icon: <ZapOff       className="w-4 h-4" />, color: "#0891b2", bg: "#ecfeff" },
  ];

  return (
    <div className="grid grid-cols-3 xl:grid-cols-5 gap-3 mb-5 r-stagger">
...
/* ════════════════════════════════════════════════════════════════
   REGISTER KPIs (for Assets Register)
════════════════════════════════════════════════════════════════ */
function AssetsRegisterKPIs({ data }: { data: ReportRow[] }) {
  const stats = useMemo(() => {
    ...
    for (const r of data) {
      total++;
      const st = String(r.asset_status || ""); statusMap[st] = (statusMap[st] || 0) + 1;
      const cat = String(r.asset_master_category || ""); if (cat) categoryMap[cat] = (categoryMap[cat] || 0) + 1;
    }
    return { total, statusMap, categoryMap };
  }, [data]);

  const topStatuses = Object.entries(stats.statusMap).sort((a, b) => b[1] - a[1]).slice(0, 4);
  const topCategories = Object.entries(stats.categoryMap).sort((a, b) => b[1] - a[1]).slice(0, 4);

  return (
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-5 r-stagger">
      {/* Total */}
      <div className="rounded-2xl border border-border bg-card px-4 py-3">
        <div className="w-8 h-8 rounded-xl bg-red-100 flex items-center justify-center mb-2" style={{ color: "#dc2626" }}>
          <Package className="w-4 h-4" />
        </div>
        <p className="text-2xl font-bold text-red-600 leading-none mb-1">{stats.total}</p>
        <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Total Assets</p>
      </div>
        ...
        <div className="space-y-1">
          {topStatuses.map(([st, n]) => (
            <div key={st} className="flex items-center justify-between gap-2">
              <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-semibold border truncate ${ASSET_STATUS_STYLES[st] || "bg-muted text-muted-foreground border-border"}`}>{st}</span>
      ...
      <div className="rounded-2xl border border-border bg-card px-4 py-3">
        ...
        <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">Top Categories</p>
        <div className="space-y-1">
          {topCategories.map(([cat, n]) => (
            <div key={cat} className="flex items-center justify-between">
              <span className="text-xs font-medium text-muted-foreground truncate">{cat}</span>
              <span className="text-xs font-bold text-foreground shrink-0">{n}</span>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-2xl border border-border bg-card px-4 py-3">
        ...
        <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">Asset Health</p>
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-muted-foreground">Active Rate</span>
            <span className="text-xs font-bold text-green-600">
              {stats.total > 0 ? Math.round(((stats.statusMap["Active"] || 0) / stats.total) * 100) : 0}%
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-muted-foreground">Maintenance</span>
            <span className="text-xs font-bold text-amber-600">{stats.statusMap["Under Maintenance"] || 0}</span>
          </div>
        </div>
      </div>
    </div>
...
function AssetDataTable({ columns, data, loading, reportId }: AssetTableProps) {
            ...
            {paged.map((row, ri) => (
              <tr key={ri} className="rpt-tr border-b border-border/50 last:border-0 transition-colors rpt-row-anim"
                ...
                {columns.map(col => {
                  ...
                  const { text, badge } = formatAssetCellValue(col, raw);
...
════════════════════════════════════════════════════════════════════ */
function AssetReportView({ report }: { report: ReportDef }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 py-5 r-fade-up">
      ...
      {hasRun && !loading && data.length > 0 && (
        report.id === "summary"
          ? <AssetsSummaryKPIs data={data} />
          : <AssetsRegisterKPIs data={data} />
      )}
...
export default function AssetReporting() {
  useRptStyles();
  const [activeId, setActiveId] = useState<string>(ASSET_REPORTS[0].id);
  const activeReport = ASSET_REPORTS.find(r => r.id === activeId)!;
    ...
    <div className="flex flex-col h-full bg-background" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-card shadow-sm shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-foreground leading-tight">Asset Report</h1>
            <p className="text-[10px] text-muted-foreground">Asset Analytics & Management</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        <AssetReportView key={activeId} report={activeReport} />
      </div>
    </div>
  );
}
