/**
 * TechnicianReporting.tsx  ·  CAFM Facility-UI
 * ─────────────────────────────────────────────────────────────────────
 * Dynamic report viewer for Frappe Script Reports.
 * Reports implemented:
 *   • Technician Summary       (technician_summary.py)
 *   • Technician Register      (technician_register.py)
 *
...
function useRptStyles() {
  useEffect(() => {
    if (document.getElementById("rpt-css-technician")) return;
    const s = document.createElement("style"); s.id = "rpt-css-technician"; s.textContent = RPT_CSS;
...
════════════════════════════════════════════════════════════════ */
const TECHNICIAN_SUMMARY_FILTERS: FilterDef[] = [
  { fieldname: "from_date",           label: "From Date", fieldtype: "Date"  },
  { fieldname: "to_date",             label: "To Date",   fieldtype: "Date"  },
  { fieldname: "staff_code",          label: "Technician", fieldtype: "Link",   options: "Resource" },
  { fieldname: "primary_area_group",  label: "Site",       fieldtype: "Link",   options: "Area Group" },
];

const TECHNICIAN_REGISTER_FILTERS: FilterDef[] = [
  { fieldname: "staff_code",          label: "Technician", fieldtype: "Link",   options: "Resource" },
  { fieldname: "primary_area_group",  label: "Site",       fieldtype: "Link",   options: "Area Group" },
  { fieldname: "designation",         label: "Skill Type", fieldtype: "Select",
    options: "\nElectrical\nMechanical\nPlumbing\nHVAC\nCivil\nFire Safety\nElevator\nGenerator\nWater Treatment\nLandscaping\nCleaning\nPest Control\nSecurity" },
  { fieldname: "shift",               label: "Shift",      fieldtype: "Select",
    options: "\nMorning\nEvening\nNight\nRotating" },
];
...
const TECHNICIAN_REPORTS: ReportDef[] = [
  {
    id: "summary",
    ...
    accent: "#059669",
    filters: TECHNICIAN_SUMMARY_FILTERS,
    description: "Aggregated technician performance metrics including jobs, response times, and productivity scores.",
  },
  {
    id: "register",
    name: "Technician Register",
    label: "Technician Register",
    icon: <FileText className="w-4 h-4" />,
    accent: "#0891b2",
    filters: TECHNICIAN_REGISTER_FILTERS,
    description: "Complete technician register with availability, skills, and assignment details.",
  },
];
...
════════════════════════════════════════════════════════════════ */
function formatTechnicianCellValue(col: ReportColumn, val: string | number | null): { text: string; badge?: string } {
  if (val === null || val === undefined || val === "") return { text: "—" };
  const s = String(val);

  if (col.fieldname === "availability")   return { text: s, badge: AVAILABILITY_STYLES[s] };
  if (col.fieldname === "is_active")      return { text: s, badge: STATUS_STYLES[s] };

  if (col.fieldtype === "Float") {
    const n = Number(val);
    if (col.fieldname === "avg_response_time") return { text: `${n.toFixed(2)}h` };
    if (col.fieldname === "productivity_score") return { text: `${n.toFixed(1)}%` };
    if (col.fieldname === "client_rating") return { text: `${n.toFixed(1)}/5` };
...
/* ════════════════════════════════════════════════════════════════
   SUMMARY STAT CARDS (for Technician Summary report)
════════════════════════════════════════════════════════════════ */
function TechnicianSummaryKPIs({ data }: { data: ReportRow[] }) {
  const totals = useMemo(() => {
    let totalTechs = 0, jobsAssigned = 0, jobsClosed = 0, openJobs = 0, responseSum = 0, responseCount = 0, attendanceSum = 0, productivitySum = 0, ratingSum = 0, ratingCount = 0;
    for (const r of data) {
      totalTechs++;
      jobsAssigned   += Number(r.jobs_assigned    || 0);
      jobsClosed     += Number(r.jobs_closed      || 0);
      openJobs       += Number(r.open_jobs        || 0);
      if (Number(r.avg_response_time) > 0) { responseSum += Number(r.avg_response_time); responseCount++; }
      attendanceSum  += Number(r.attendance_days  || 0);
      productivitySum+= Number(r.productivity_score || 0);
      if (Number(r.client_rating) > 0) { ratingSum += Number(r.client_rating); ratingCount++; }
    }
    const avgResponse = responseCount > 0 ? (responseSum / responseCount).toFixed(2) : "—";
    const avgProductivity = totalTechs > 0 ? (productivitySum / totalTechs).toFixed(1) : "—";
    const avgRating = ratingCount > 0 ? (ratingSum / ratingCount).toFixed(1) : "—";
    return { totalTechs, jobsAssigned, jobsClosed, openJobs, avgResponse, attendanceSum, avgProductivity, avgRating };
  }, [data]);

  const cards = [
    { label: "Technicians",    value: totals.totalTechs,     icon: <Wrench        className="w-4 h-4" />, color: "#059669", bg: "#ecfdf5" },
    { label: "Jobs Assigned",  value: totals.jobsAssigned,   icon: <ClipboardList className="w-4 h-4" />, color: "#0891b2", bg: "#ecfeff" },
    { label: "Jobs Closed",    value: totals.jobsClosed,     icon: <CheckCircle2  className="w-4 h-4" />, color: "#10b981", bg: "#f0fdf4" },
    { label: "Open Jobs",      value: totals.openJobs,       icon: <Clock         className="w-4 h-4" />, color: "#f59e0b", bg: "#fffbeb" },
    { label: "Avg Response",   value: totals.avgResponse,    icon: <Timer         className="w-4 h-4" />, color: "#8b5cf6", bg: "#f5f3ff" },
    { label: "Avg Rating",    value: totals.avgRating,      icon: <Star          className="w-4 h-4" />, color: "#ea580c", bg: "#fff7ed" },
  ];
...
/* ════════════════════════════════════════════════════════════════
   REGISTER KPIs (for Technician Register)
════════════════════════════════════════════════════════════════ */
function TechnicianRegisterKPIs({ data }: { data: ReportRow[] }) {
  const stats = useMemo(() => {
    ...
    for (const r of data) {
      total++;
      const st = String(r.is_active || ""); statusMap[st] = (statusMap[st] || 0) + 1;
      const av = String(r.availability || ""); if (av) availabilityMap[av] = (availabilityMap[av] || 0) + 1;
      const sk = String(r.designation || ""); if (sk) skillMap[sk] = (skillMap[sk] || 0) + 1;
      const sh = String(r.shift || ""); if (sh) shiftMap[sh] = (shiftMap[sh] || 0) + 1;
    }
    return { total, statusMap, availabilityMap, skillMap, shiftMap };
  }, [data]);

  const topSkills = Object.entries(stats.skillMap).sort((a, b) => b[1] - a[1]).slice(0, 4);
  ...
  return (
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-5 r-stagger">
      ...
      <div className="rounded-2xl border border-border bg-card px-4 py-3">
        ...
        <p className="text-2xl font-bold text-green-600 leading-none mb-1">{stats.total}</p>
        <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Total Technicians</p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card px-4 py-3">
        ...
        <div className="space-y-1">
          {Object.entries(stats.statusMap).map(([st, n]) => (
      ...
      <div className="rounded-2xl border border-border bg-card px-4 py-3">
        ...
        <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">Availability</p>
        <div className="space-y-1">
          {Object.entries(stats.availabilityMap).map(([av, n]) => (
            <div key={av} className="flex items-center justify-between gap-2">
              <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-semibold border truncate ${AVAILABILITY_STYLES[av] || "bg-muted text-muted-foreground border-border"}`}>{av}</span>
              <span className="text-xs font-bold text-foreground shrink-0">{n}</span>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-2xl border border-border bg-card px-4 py-3">
        ...
        <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground mb-2">Top Skills</p>
        <div className="space-y-1">
          {topSkills.map(([sk, n]) => (
            <div key={sk} className="flex items-center justify-between">
              <span className="text-xs font-medium text-muted-foreground truncate">{sk}</span>
              <span className="text-xs font-bold text-foreground shrink-0">{n}</span>
            </div>
          ))}
        </div>
...
function TechnicianDataTable({ columns, data, loading, reportId }: TechnicianTableProps) {
            ...
            {paged.map((row, ri) => (
              <tr key={ri} className="rpt-tr border-b border-border/50 last:border-0 transition-colors rpt-row-anim"
                ...
                {columns.map(col => {
                  ...
                  const { text, badge } = formatTechnicianCellValue(col, raw);
...
════════════════════════════════════════════════════════════════ */
function TechnicianReportView({ report }: { report: ReportDef }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 py-5 r-fade-up">
      ...
      {hasRun && !loading && data.length > 0 && (
        report.id === "summary"
          ? <TechnicianSummaryKPIs data={data} />
          : <TechnicianRegisterKPIs data={data} />
      )}
...
/* ════════════════════════════════════════════════════════════════
   MAIN PAGE
════════════════════════════════════════════════════════════════ */
export default function TechnicianReporting() {
  useRptStyles();
  const [activeId, setActiveId] = useState<string>(TECHNICIAN_REPORTS[0].id);
  const activeReport = TECHNICIAN_REPORTS.find(r => r.id === activeId)!;
    ...
    <div className="flex flex-col h-full bg-background" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-card shadow-sm shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-foreground leading-tight">Technician Report</h1>
            <p className="text-[10px] text-muted-foreground">Technician Performance Analytics</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        <TechnicianReportView key={activeId} report={activeReport} />
      </div>
    </div>
  );
}
